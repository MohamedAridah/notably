...
const TrashUI = ({ notebooks, notes = [] }: TrashUIProps) => {
  ...
  const tHLines = useTranslations("HeadLines");
    ...
    <>
      ...
      >
        <Search
          ...
          aria-label={tSForm("placeholder.trashed")}
          placeholder={tSForm("placeholder.trashed")}
        />
        <ViewController />
      </div>
        ...
        <p className="mb-4">
          {displayState.isEmpty
            ...
            : tSForm("state.withMatchMessage", {
                count:
                  displayState.notebooks.length + displayState.notes.length,
              })}
          ...
          <p className="text-muted-foreground text-center py-8" role="status">
            {displayState.isSearching
              ? tSForm("state.noMatchMessage", {
                  term: tCommon("item", { count: 2 }),
                })
              : tSForm("state.empty")}
          </p>
        ) : (
          renderTrashContent(displayState, view, tHLines)
        )}
      </div>
    </>
  );
